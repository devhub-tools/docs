...
Actions appear as buttons on each row of a query panel's results. When clicked, they trigger a workflow, populating each of the workflow's inputs either from the clicked row's columns or from values the user is prompted for.
...
<Properties>
  ...
  <Property name="input_mappings" type="array">
    Array of mappings that populate the workflow's inputs. One mapping is expected per workflow input. See [Input mapping structure](#input-mapping-structure) below.
  </Property>
</Properties>

### Input mapping structure

Each mapping fills a single workflow input. By default the value comes from a column of the clicked row. Set `prompt_user` to `true` to instead ask the user for the value when they click the button — in that case the dashboard opens a modal with one field per prompted input and only starts the workflow once the user submits it.

<Properties>
  <Property name="workflow_input_key" type="string">
    The key of the workflow input this mapping populates.
  </Property>
  <Property name="column" type="string">
    The result column whose value is passed in. Used when `prompt_user` is false.
  </Property>
  <Property name="prompt_user" type="boolean">
    Whether to prompt the user for this value when the action is clicked instead of reading it from the row. Defaults to false.
  </Property>
  <Property name="prompt" type="string">
    The label shown above the field in the prompt modal. Used when `prompt_user` is true, and falls back to `workflow_input_key` when blank.
  </Property>
</Properties>
            ...
            "actions": [
              {
                ...
                "input_mappings": [
                  ...
                  },
                  {
                    ...
                    "prompt_user": true,
                    "prompt": "Why are you restarting this host?"
                  }
                ]
              }
            ],
      ...
      "panels": [
        {
          ...
          "actions": [
            {
              "id": "6ad2b0e2-1f2e-4a3f-9a1d-2f1c9a0b7e11",
              "label": "Restart host",
              "workflow_id": "wf_123456789",
              "input_mappings": [
                {
